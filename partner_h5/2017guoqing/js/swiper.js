(function() {
    ...
    var Swiper = function(container, params) {
        ...
        if (['cube', 'coverflow', 'flip', 'flip2'].indexOf(s.params.effect) >= 0) {
        ...
        if (s.params.effect === 'fade' || s.params.effect === 'flip' || s.params.effect === 'flip2') {
            s.params.slidesPerView = 1;
            s.params.slidesPerColumn = 1;
            s.params.slidesPerGroup = 1;
            s.params.watchSlidesProgress = true;
        ...
        s.updateActiveIndex = function() {
            ...
            for (i = 0; i < s.slidesGrid.length; i++) {
                if (typeof s.slidesGrid[i + 1] !== 'undefined') {
                    if (translate >= s.slidesGrid[i] + 10 && translate < s.slidesGrid[i + 1] + 10) {
                        newActiveIndex = i;
                    } else if (translate > s.slidesGrid[i] && translate < s.slidesGrid[i + 1] + 10) {
                        newActiveIndex = i + 1;
                    }
                } else {
                ...
                setTranslate: function() {
                    for (var i = 0; i < s.slides.length; i++) {
                        var slide = s.slides.eq(i);
              			var slidenext;
                        var progress = slide[0].progress;
                        if (s.params.flip2.limitRotation) {
                            progress = Math.max(Math.min(slide[0].progress, 1), -1);
                        }
                        var offset = slide[0].swiperSlideOffset;
                        var rotate = -120 * progress
                          , rotateY = rotate
                          , rotateX = 0
                          , tx = -offset
                          , ty = 0;
                        ...
                        slide[0].style.zIndex = -Math.abs(Math.round(progress)) + s.slides.length;
                        if (s.params.flip2.slideShadows) {
                        ...
                        if (slide.hasClass("swiper-slide-next")) {
                            if (s.activeIndex == 0) {
                                if (rotateY == 90) {
                                    slide.transform('translate3d(' + tx + 'px, ' + ty + 'px, 0px) rotateX(' + rotateX + 'deg) rotateY(' + 90 + 'deg)');
                                } else {
                                    slide.transform('translate3d(' + tx + 'px, ' + ty + 'px, 0px) rotateX(' + rotateX + 'deg) rotateY(' + 0 + 'deg)');
                                }
                            } else {
                                slide.transform('translate3d(' + tx + 'px, ' + ty + 'px, 0px) rotateX(' + rotateX + 'deg) rotateY(' + 0 + 'deg)');
                            }
